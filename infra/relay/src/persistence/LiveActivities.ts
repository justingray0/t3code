        ...
        .pipe(
          Effect.flatMap((rows) =>
            Effect.forEach(
              ...
              (row) =>
                Effect.all({
                  preferences_json: stringifyJsonValue(row.preferences_json),
                  last_aggregate_json:
                    row.last_aggregate_json === null
                      ? Effect.succeed(null)
                      : stringifyJsonValue(row.last_aggregate_json),
                }).pipe(
                  Effect.map((json) => ({
                    ...row,
                    ...json,
      ...
      function* (input) {
        ...
        const aggregateJson =
          input.aggregate === null
            ? null
            : yield* parseJsonString<RelayAgentActivityAggregateState>(
                yield* encodeRelayAgentActivityAggregateStateJson(input.aggregate),
              );
